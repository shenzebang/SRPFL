...
def local_update(local, net, idx, representation_keys, lr):
    w_local, loss, _ = local.train(net=net, idx=idx, representation_keys=representation_keys, lr=lr)
    return w_local, loss
...
if __name__ == '__main__':
    ...
    for iter in trange(args.epochs):
        ...
        locals = [LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users_train[idx]) for idx in idxs_users]
        results = ray.get([ray_dispatch.remote(local, net_local, idx, representation_keys, args.lr)
                           for local, net_local, idx in zip(locals, net_locals, idxs_users)])
