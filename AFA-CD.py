...
if __name__ == '__main__':
    ...
    for system_time in trange(args.maximum_system_time):
        ...
        if len(user_ready) != 0:
            # results = ray.get([
            #     ray_dispatch.remote(LocalUpdateFEDAVG(args=args, dataset=dataset_train, idxs=dict_users_train[user.uid]),
            #                         user.current_model)
            #     for user in user_ready
            # ])

            results = [dispatch(LocalUpdateFEDAVG(args=args, dataset=dataset_train, idxs=dict_users_train[user.uid]), user.current_model)
                        for user in user_ready]
    ...
    # if args.frac == 1:
    #     FT_save_file = f"./save/{args.dataset}-{args.shard_per_user}-{args.num_users}/FEDAFA-{args.description}-FT-{args.repeat_id}-{args.hyper_setting}.csv"
    # else:
    #     FT_save_file = f"./save/{args.dataset}-{args.shard_per_user}-{args.num_users}/FEDAFA-partial-{args.description}-FT-{args.repeat_id}-{args.hyper_setting}.csv"
    ...
    if args.frac == 1:
        global_save_file = f"./save/{args.dataset}-{args.shard_per_user}-{args.num_users}/AFA-CD-{args.description}-global-{args.repeat_id}-{args.hyper_setting}.csv"
    else:
        global_save_file = f"./save/{args.dataset}-{args.shard_per_user}-{args.num_users}/AFA-CD-partial-{args.description}-global-{args.repeat_id}-{args.hyper_setting}.csv"
